...
angular.module('estudiantes').controller('EstudiantesController', ['$scope', '$stateParams', '$location', '$filter', '$http', '$sce', 'Authentication', 'Estudiantes', '$upload', 'Notas', 'GetNotas', 'GetAdmitidos', 'Decimo', 'Undecimo', 'Nacionalidad', 'Reports',
	function($scope, $stateParams, $location, $filter, $http, $sce, Authentication, Estudiantes, $upload, Notas, GetNotas, GetAdmitidos, Decimo, Undecimo, Nacionalidad, Reports) {
        ...
        $scope.lista_reportes = Reports.getReportList();
        ...
        $scope.$watch('reporte', function(reporte){
            ...
            if(reporte.val === 1){
                serviceReport.estudiantes_decimo.$promise.then(function (estudiantes) {
                    var result = serviceReport.absenteeList(estudiantes)
            ...
            //wIP Ask what to do with eleventh grade. PO maybe.
                ...
                serviceReport.estudiantes_decimo.$promise.then(function (estudiantes) {
                    var resultDec = serviceReport.personalInfoList(estudiantes);
            ...
            //WIP I could define var reports up and call the specific report where it goes. WIP.
                ...
                serviceReport.estudiantes_undecimo.$promise.then(function(estudiantes){
                    var result = serviceReport.listOfStudentsForLibrary(estudiantes);
                    if(result['Data'].length > 0){
                        $scope.generatePDF(result["Filename"], result["Header"], result["Title"], result["Columns"], result["Data"], result["StartY"], result["Save"]);
                    }
                });
                ...
                serviceReport.estudiantes_undecimo.$promise.then(function(estudiantes){
                    var result = serviceReport.listForOlimpicsParticipation(estudiantes);
                    if(result['Data'].length > 0){
                        $scope.generatePDF(result["Filename"], result["Header"], result["Title"], result["Columns"], result["Data"], result["StartY"], result["Save"]);
                    }
                });
